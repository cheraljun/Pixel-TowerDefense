...
export default class Game {
	constructor(dom, sprites) {
		...
		this.loadGrid();
		this.loadPaths();
		// 确保每个可通行格子都成功获得 shortestAdjacent
		for (let c = 0; c < this.numCols; c++) {
			for (let r = 0; r < this.numRows; r++) {
				const cell = this.grid[c][r];
				if (!cell.occupied && cell !== this.goal && cell.smallestAdjacent == null) {
					cell.getShortestRoute();
				}
			}
		}
		...
		this.actions = new Actions(this, dom, this.tiles);
	}
	...
	createTiles() {
		...
		for (let i = 0; i < 4; i++) {
			...
			// use canvas tile instead of PNG
			const tileCanvas = this.sprites.tileCanvas?.[tile.type];
			if (tileCanvas) {
				tile.appendChild(tileCanvas);
			}

			// 不再在塔牌下方显示价格文字，避免与按钮信息重复
			...
			for (let r = 0; r < this.numRows; r++) {
				this.grid[c].push(
					new Cell(
						...
						this.cellSize,
						this.context,
						this.sprites.board.wall,
						this.sprites.board.wallSelected,
						c,
						r
					)
				);
	...
	addBorderCell(col, row) {
		...
		cell.occupied = true;
		cell.static = true;
		cell.img = this.sprites.board.border;
		this.border.push(cell);
	}

	initPosts() {
		const startCol = 2 + Math.floor(Math.random() * 3);
		const startRow = 2 + Math.floor(Math.random() * (this.numRows - 3));
		this.start = this.grid[startCol][startRow];
		this.start.img = this.sprites.board.start;
		this.start.static = true;
		const goalCol = this.numCols - (2 + Math.floor(Math.random() * 3));
		const goalRow = 2 + Math.floor(Math.random() * (this.numRows - 3));
		this.goal = this.grid[goalCol][goalRow];
		this.goal.value = 0;
		this.goal.img = this.sprites.board.goal;
	...
	initBlocks() {
		...
		for (let i = 0; i < this.numBlocks; i++) {
			const randCol = 2 + Math.floor(Math.random() * (this.numCols - 3));
			const randRow = 2 + Math.floor(Math.random() * (this.numRows - 3));
			const cell = this.grid[randCol][randRow];
